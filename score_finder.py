...
def best_score_finder(ms1_array, precursor_mz_score, scan_number: int, maximal_error:int):
    #If the ms1 array is empty do nothing
    #print(f"MS1 scan number {scan_number} length is {len(ms1_array)} with precursor m/z {precursor_mz_score}")
    if len(ms1_array) == 0:
        return precursor_mz_score, 0, 0, 0
    else:
